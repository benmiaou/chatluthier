...
function refreshToken(req, res) {
  ...
  try {
    ...
    const accessToken = jwt.sign(
      { userId: payload.userId, pseudo: userIdentifier, isAdmin },
      accessTokenSecret,
      { expiresIn: '1h' }
    );
...
async function registerWithPseudo(req, res) {
  try {
    ...
    if (!pseudo || !password || !secretQuestion || !secretAnswer) {
      return res
        .status(400)
        .json({ error: 'Pseudo, password, secret question, and secret answer are required' });
    }

    // Check if pseudo already exists (case-insensitive)
    const existingUser = await db.queryOne('SELECT id FROM users WHERE LOWER(pseudo) = LOWER(?)', [
      pseudo,
    ]);
    if (existingUser) {
    ...
    fs.writeFileSync(path.join(userDir, 'presets.json'), JSON.stringify({}, null, 2));
...
async function requestPasswordReset(req, res) {
  try {
    ...
    if (!pseudo || !secretAnswer || !newPassword) {
      return res
        .status(400)
        .json({ error: 'Pseudo, secret answer, and new password are required' });
    }

    // Find user with secret question (case-insensitive)
    const user = await db.queryOne(
      'SELECT id, password_hash, secret_question, secret_answer_hash FROM users WHERE LOWER(pseudo) = LOWER(?)',
      [pseudo]
    );
    if (!user) {
...
async function loginWithPseudo(req, res) {
  try {
    ...
    // Find user (case-insensitive)
    const user = await db.queryOne(
      'SELECT id, pseudo, password_hash, is_admin FROM users WHERE LOWER(pseudo) = LOWER(?)',
      [pseudo]
    );
    if (!user) {
