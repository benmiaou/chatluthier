...
class JSONtoSQLMigrator {
    ...
    async migrateServerSounds() {
        const soundTypes = [
            { name: 'ambianceSounds', file: 'ambianceSounds.json', table: 'ambiance_sounds', hasImageFile: true },
            { name: 'backgroundMusic', file: 'backgroundMusic.json', table: 'background_sounds', hasImageFile: true },
    ...
    async insertSound(soundData, soundType) {
        return new Promise((resolve, reject) => {
            ...
            if (soundType.table !== 'background_sounds' && Array.isArray(contexts) && contexts.length > 0 && Array.isArray(contexts[0])) {
