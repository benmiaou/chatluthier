...
export function CreditsModal({ opened, onClose }: Readonly<CreditsModalProps>): React.JSX.Element {
  ...
  useEffect(() => {
    ...
    const fetchAll = async () => {
      try {
        ...
        setCredits(
          [
            ...bg.map((s: SoundEntry) => toEntry(s, 'Background Music')),

            ...amb.map((s: SoundEntry) => toEntry(s, 'Ambiance Sounds')),

            ...sb.map((s: SoundEntry) => toEntry(s, 'Soundboard')),
  ...
  const byCategory = credits.reduce<Record<string, CreditEntry[]>>(
    (acc: Record<string, CreditEntry[]>, c: CreditEntry) => {
        ...
        <div style={{ flex: 1, overflow: 'auto', marginTop: '1rem', padding: '0 1rem' }}>
          <Tabs.Panel value="background">
            {byCategory['Background Music']?.length > 0 ? (
              <Stack gap="md">
                {byCategory['Background Music'].map((item: CreditEntry) => (
                  <Stack
                    key={item.name}
                    gap="xs"
                    style={{
          ...
          <Tabs.Panel value="ambiance">
            {byCategory['Ambiance Sounds']?.length > 0 ? (
              <Stack gap="md">
                {byCategory['Ambiance Sounds'].map((item: CreditEntry) => (
                  <Stack
                    key={item.name}
                    gap="xs"
                    style={{
          ...
          <Tabs.Panel value="soundboard">
            {byCategory.Soundboard?.length > 0 ? (
              <Stack gap="md">
                {byCategory.Soundboard.map((item: CreditEntry) => (
                  <Stack
                    key={item.name}
                    gap="xs"
                    style={{
